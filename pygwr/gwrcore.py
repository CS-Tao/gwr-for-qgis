...
class GWRResultSet(list):
    ...
    def getTableInfo(self):
        ...
        # Write out the header line.
        for i in range(ndims):
            tableHeader.append("EST_PT_%d" % (i+1))
        n = len(p0.params)
        # Add the sample
        for i in range(n-1):
            tableHeader.append("SAMPLE_%d" % (i+1))
        # Add the parameters
        for i in range(n):
            tableHeader.append("PARAM_%d" % (i+1))
        # Add the t-values
        for i in range(n):
            tableHeader.append("TVAL_%d" % (i+1))
        # Add the p-values
        for i in range(n):
            tableHeader.append("PVAL_%d" % (i+1))
        # Add the target and prediction
        tableHeader.append("TARGET")
        tableHeader.append("PREDICT")
        for r in self:
            rowData = []
            for i in range(ndims):
                rowData.append("%f" % r.estimation_pt[i])
            for i in range(n-1):
                rowData.append("%f" % r.sample[i])
            for i in range(n):
                rowData.append("%f" % r.params[i])
            for i in range(n):
                rowData.append("%f" % r.result.tvalues[i])
            for i in range(n):
                rowData.append("%f" % r.result.pvalues[i])
            if r.idx != None:
                rowData.append("%f" % r.gwr.targets[r.idx])
            else:
                rowData.append("")
            rowData.append("%f" % r.prediction)
